% === Folleto COVAO - Soluciones visibles ===
...
\usepackage{pifont}
\usepackage{ocgx2} % ya no se usará en modo visible, pero lo dejo cargado
...
% ===== Interruptor global para soluciones =====
\newif\ifshowsolutions
\showsolutionstrue   % <-- Cambiar a \showsolutionsfalse si querés volver al modo clickeable
...
\ifshowsolutions
  ...
  \renewenvironment{solucionclick}{%
    \refstepcounter{solnum}%
    \par\noindent\textbf{Solución \#\thesolnum}\\[2pt]%
    \begin{solucionbox}%
  }{%
    \end{solucionbox}\par\medskip
  }
\else
  % --- Modo CLICK-TO-REVEAL ---
  \renewenvironment{solucionclick}{%
    \refstepcounter{solnum}%
...
\newcommand{\titulo}{
  ...
  {\small Álgebra, aritmética y razonamiento cuantitativo (con soluciones)}\\[6pt]
